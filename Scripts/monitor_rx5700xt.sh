#!/bin/bash
# ==============================================================================
# Monitor de Telemetria em Tempo Real - AMD Radeon RX 5700 XT (Navi 10)
# ==============================================================================
# Uso: ./monitor_rx5700xt.sh  (ou arraste para o Terminal)
# Para sair: Ctrl + C
# ==============================================================================
...
while true; do
  DATA=$(ioreg -l -w0 -r -c AMDRadeonX6000_AMDNavi10GraphicsAccelerator 2>/dev/null)
  
  if [ -z "$DATA" ]; then
    clear
    echo -e "${RED}Erro: Acelerador gráfico Navi 10 (RX 5700 XT) não encontrado.${NC}"
    echo "Aguardando GPU..."
    sleep 2
    continue
  fi
  ...
  clear
  echo -e "${BLUE}======================================================${NC}"
  echo -e "${BOLD}${CYAN}   TELEMETRIA AO VIVO - AMD RADEON RX 5700 XT         ${NC}"
  echo -e "${BLUE}======================================================${NC}"
